...
| Дата | Суть |
|------|------|
| 2026-09-02 | В таблице блока «Ценные бумаги» добавлена колонка-чекбокс «Активна» (is_active через PUT /api/logic-securities/:id); неактивная строка гаснет и исключается из торговли. «Тень/реал→тень» — отдельный признак real_trading_paused (бейдж), чекбокс назван «Активна». Блок «Ценные бумаги» — между «Торговыми периодами» и блоками позиций. web + installers v1.0.190 (не закоммичено) |
