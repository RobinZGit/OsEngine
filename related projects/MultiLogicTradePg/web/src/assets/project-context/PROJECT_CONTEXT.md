...
**Последнее обновление:** 2026-09-26 — **терминал: галочка «закрывать позицию по сигналу логики» перенесена из верхнего бара на бар каждой бумаги (шапку полосы), включена по умолчанию; при включённой — автозакрытие всей позиции бумаги по сигналу/стоп-лоссу/тейк-профиту любой сигнализирующей логики, при выключенной — только ручное закрытие кнопкой «Закрыть позицию» (#919).** Инструкции — #919. Запрос Sergey: «галочку на том же баре закрывать позицию по сигналу логики — она должна быть включена; если включена — позиция автоматически закрывается на всё количество бумаги по сигналу любой логики, включённой на сигнализацию, в том числе по её любым стоп-лоссам и тейк-профитам. Хочу иметь возможность включить/выключить на баре, чтобы закрывать вручную либо оставить включённой и пусть логика решает сама». Сделано: (1) **Терминал** — `web/src/app/terminal/terminal.component.ts`: глобальный чекбокс `autoCloseOnLogicSignal` из верхнего бара (#916) удалён из компонента, html и настроек; контроль перенесён на бары полос. `PanelModel` и `TerminalPanelState` получили `auto_close_on_logic_signal` (по умолчанию **true**): `buildPanel(securityId, tf, height, signalEvent, ids, collapsed = true, autoCloseOnLogicSignal = true)`; `loadSavedState` передаёт `st.auto_close_on_logic_signal` (для старых состояний без флага — остаётся включённым); `applyLogicSignals` ведёт `target: PanelModel` (существующая либо созданная сигналом полоса) и при `target.auto_close_on_logic_signal && securityRemainderQty ≠ 0` шлёт `closeSignalPulse` на полосу — закрытие всей позиции маркетом; обработка сигнала/закрытия/стоп-лосса/ТП ЛЮБОЙ логики с включёнными сигналами по бумаге осталась на SQL/API из #916 (`logic_close_security_positions_market()` + `signal_kind='close'`). Новый `onPanelAutoCloseChange(value, uid)` пишет флаг в полосу и сохраняет; `buildPayload` выносит `auto_close_on_logic_signal` в состояние полосы. (2) **Панель** — `web/src/app/terminal/terminal-panel.component.ts` (+`.html`/`.css`): `@Input() autoCloseOnLogicSignal = true`, `@Output() autoCloseChange`, чекбокс `tpanel-auto-close` на баре (виден всегда, не только при сигнале), подсказка объясняет включённую/выключенную семантику; `onAutoCloseChange(event)` читает состояние чекбокса и уведомляет терминал. Проверено: unit **136/136** (+3 теста чекбокса: по умолчанию включён и виден без сигнала; снятый рисуется; переключение эмитит новое значение), `ng build` prod OK (только бюджетные warning'и). SQL и API не менялись. Установщики = v1.0.294.

**Предыдущее обновление:** 2026-09-26 — **терминал: при наведении на бар с бумагой (шапка полосы) показывается хинт — какой логикой подан сигнал и сколько времени прошло (только что, секунды/минуты/часы/дни назад) (#918).** Инструкции — #918. Запрос Sergey: «в том же списке при наведении на бар с бумагой сверху сделай хинт — нужно писать, какой логикой этот сигнал был подан и сколько там минут/секунд/часов назад». Сделано: (**панель** `web/src/app/terminal/terminal-panel.component.ts` + `.html`) — getter `signalTimeAgo`: по `signalEvent.bar_dt` (бару сигнала логики) считает разницу с текущим временем: <5 с — «только что», далее секунды/минуты/часы/дни назад с русским склонением через `pluralWord` («1 минуту назад», «2 минуты назад», «5 минут назад»; «час/часа/часов», «день/дня/дней»); getter `signalBarHint` = `signalBadgeTitle` (логика, дата бара, таймфрейм) + «. Подано N назад». Хинт навешан `[title]="signalEvent ? signalBarHint : ''"` на корень `.tpanel-head` — показывается при наведении на бар с бумагой в списке терминала (в т.ч. в свёрнутых полосах), без сигнала title пуст. «Подано … назад» обновляется вместе с перерисовкой (живой опрос свечей каждые 15 с). Проверено: unit **133/133** (+3 теста: «только что»/«1 минуту назад»/«2 часа назад», хинт с логикой и «подано», наличие/отсутствие title на баре), `ng build` prod OK (только бюджетные warning'и). SQL и API не менялись. Установщики = v1.0.293.
