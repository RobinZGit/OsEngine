...
**Последнее обновление:** 2026-09-06 — **инверсия логики: убрана красная окантовка строки, вместо неё прямо под именем логики жирное красное слово INVERSION.** Ранее `.row-inversion` (outline `#ef4444` + inset box-shadow) красными рамками сверху/снизу выделял строку — при двух соседних инвертированных логиках «хорошая» логика между ними тоже выглядела обрамлённой (оптическая иллюзия). Исправлено: CSS `.row-inversion` и биндинг `[class.row-inversion]` в `logics.component.html` удалены; в `.logic-name-cell` сразу после `.logic-name-main` добавлен `<span class="logic-inversion-label"><span class="logic-inversion-arrow">▲▼</span> INVERSION IS ON</span>` (0.7rem, weight 800, uppercase, letter-spacing 0.08em, `#dc2626`, title «Инверсия включена — стороны сделок развёрнуты»), примечание и запись сигналов идут ниже как раньше. Бирюзовая рамка `row-portfolio-paused` не трогалась. `ng build` prod OK (только бюджетные warning'и). Установщики **v1.0.213** (не коммичено).
