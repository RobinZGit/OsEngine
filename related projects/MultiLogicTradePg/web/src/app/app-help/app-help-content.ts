...
• Стоп-лосс / тейк-профит — в списке типов видны все варианты; недоступны для выбора (серые): SL «портфель с обновлением», любые TP по всему портфелю логики. Доступны: SL security / security_resume / security_inversion / portfolio; TP только по бумаге. «Инверсия при повторной просадке» (security_inversion): один порог «Значение» (%). Старт с нуля; рост по логике не включает инверсию. Просадка ≥ % → закрытие и shadow; когда shadow возвращается к нулю (пику на стопе) — бой снова и флаг инверсии переключается; тот же % действует на инвертированной логике; снова стоп → shadow → снова ноль → снятие инверсии. Уже сохранённые недоступные типы остаются в таблице.
