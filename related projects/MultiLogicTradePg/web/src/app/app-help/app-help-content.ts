...
• Для быстрого теста OPT можно временно оставить 3–5 бумаг вместо всего портфеля.`,
  ...
  {
    id: 'signal-formula-edit',
    title: 'Редактор формулы сигнала',
    body: `Формула сигнала редактируется прямо в поле «Формула сигнала» на логике: можно менять параметры индикатора (@LINREG(period=20,…) → period=50), серию (series=LOWER/UPPER/MIDDLE) и само условие. Сохранение — по выходу из поля или Ctrl+Enter.

Кнопка «?» открывает справку по редактированию: она есть в шапке блока «Сигналы на логике» (рядом с названием, сворачивает/разворачивает панель со справкой) и в правом верхнем углу каждого поля формулы. В справке: формат формулы, арифметика и каталог индикаторов с их параметрами.

Одинаковые сигналы (#831): ограничение уникальности снято — можно добавить сколько угодно сигналов одного типа (например два «открытие long по течению» с разными параметрами); каждый сигнал — самостоятельная строка со своей формулой, все активные сигналы группы работают как AND.

Переменные условия:
• pp — цена Close текущего бара;
• VALUE — значение серии индикатора из @CODE(...) (series=…).

Арифметика (применительно к рядам/векторам):
• + − покомпонентное сложение / вычитание;
• * свёртка рядов; для значения одного бара — умножение;
...
• @RSI(period=14) VALUE - 50 > 25 — RSI выше 75.

Параметры основных индикаторов:
• RSI(period=14,series=VALUE);
• MACD(fast=12,slow=26,signal=9,series=MACD);
• STOCH(k=14,d=3,smooth=3,series=K);
• BB / LINREG / SQUARE(period=20,std_dev=2,series=MIDDLE);
• ATR(period=14,series=VALUE);
• OPT(key,percent) — пометить параметр для оптимизации на лету (см. главу «Формулы сигналов и OPT()»).`,
  },
  {
    id: 'real-trading',
...
Защита от займа при резком движении (параметры логики, «Расчёт лота»):
• «Буфер цены исполнения, %» — количество Open считается по цене × (1 + буфер): при гэпе против позиции заявка меньше. Пусто = выкл.
• «Макс. гэп входа, %» — перед Open свежая цена (стоп-ТФ) сравнивается с ценой сигнала; отклонение больше порога → вход пропускается до следующей свечи (trade.gap_skip в журнале). Пусто = выкл.
Оба работают одинаково для long и short. Тест/бэктест исполняется по цене закрытия свечи — эти параметры только про живое исполнение.
