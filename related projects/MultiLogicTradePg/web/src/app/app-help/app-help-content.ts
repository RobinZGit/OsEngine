...
• Стоп-лосс / тейк-профит — в списке типов видны все варианты; недоступны для выбора (серые): SL «портфель с обновлением», любые TP по всему портфелю логики. Доступны: SL security / security_resume / security_inversion / portfolio; TP только по бумаге. «По бумаге и стороне с возобновлением» (security_resume): просадка стороны ≥ % → close + shadow; возобновление при достижении цели track. Параметр resume_sl_no_reduce: при повторном стопе цель не ниже прежнего максимума (HWM). «Инверсия при повторной просадке» (security_inversion): один порог «Значение» (%). Старт с нуля; рост по логике не включает инверсию. Просадка ≥ % → закрытие и shadow; когда shadow возвращается к нулю (пику на стопе) — бой снова и флаг инверсии переключается; тот же % действует на инвертированной логике; снова стоп → shadow → снова ноль → снятие инверсии. Уже сохранённые недоступные типы остаются в таблице.
• Ценные бумаги — портфель логики; лотность учитывается при открытии.
• Позиции — боевые сделки; Тестирование — бэктест с эквити и рейтингами на бумаге; кнопки Экспорт / Отчёт / Запуск·Стоп; галочка «Оптимизировать» — в том же прогоне бумажные ветки сетки параметров (шаг, итерации ±, лимит 81), Отчёт OPT; на панели «Сигналы» — «Применить лучшие OPT» / «Параметры по умолчанию».
...
• Один прогон: чемпион = базы по умолчанию (эквити/сделки как обычно); сетка = бумажные opt_lane без mid-run promote; в конце — Отчёт OPT и rank FinRes.
• На панели «Сигналы»: «Применить лучшие OPT» (записать базы в формулы) и «Параметры по умолчанию» (как Сброс OPT).
