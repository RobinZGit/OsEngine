...
Таймфрейм сигнала (#843): параметр tf= внутри @CODE(...).
  @SMA(tf=M1*7,period=50) pp >= VALUE   — «семиминутный» ТФ (7 × M1)
  @LINREG(tf=H1,period=100) …            — каталожный ТФ
• База — из каталога: M1…M30, H1…H12, D1. Множитель — целое ≥1.
• Пусто/нет tf= → сигнал считается на ТФ логики.
• Сигнал оценивается на последнем закрытом баре своего ТФ, который
  закрылся не позже закрытия текущего бара ТФ логики (без заглядывания).
• Производные интервалы (M7 и т.п.) строятся ресемплом из M1: у T-Bank
  глубина минутки ограничена — история такого ТФ накапливается со временем.
