-- Seed / refresh «LinReg Fade Twice Optimized» on an existing DB (idempotent).
INSERT INTO logics (name, account_id, is_enabled, note)
SELECT
    'LinReg Fade Twice Optimized',
    a.id,
    FALSE,
    'Like LinReg Fade with OPT(std_dev,10) and OPT(period,10) on-the-fly optimization.'
FROM accounts a
...
JOIN logics dst ON dst.name = 'LinReg Fade Twice Optimized'
