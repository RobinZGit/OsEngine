-- Трендовая логика CMO (моментум Чанде) + Стохастик
-- Лонг по тренду: CMO>0 И %K>50; шорт по тренду: CMO<0 И %K<50.
-- Выход — на возврате к нулю/50. Те же 39 бумаг (акции+фонды), что у LinReg Fade Trend.
-- СПЯЩАЯ версия (is_enabled=FALSE): бэктест #367 ≈ −384 528₽ — признана неудачной.
...
SELECT
    ...
    a.id,
    FALSE,
    'Тренд: CMO(14) + Стохастик %K(14,3,3). Лонг CMO>0 и K>50, шорт CMO<0 и K<50. Только акции и фонды. СПЯЩАЯ (неудачная), торговые периоды MOEX.'
FROM accounts a
...
-- Параметры как у CMO Stoch Counter (M15, 10%, до 15 позиций, MOEX-периоды)
...
JOIN logics dst ON dst.name = 'CMO Stoch Trend'
WHERE src.name = 'LinReg Fade Trend'
ON CONFLICT (logic_id, security_id) DO UPDATE SET
    is_active = EXCLUDED.is_active,
    display_order = EXCLUDED.display_order;

-- Стопы как у CMO Stoch Counter: SL 1% по бумаге, TP 5% портфеля (продление по возобновлению)
...
FROM logics l WHERE l.name = 'CMO Stoch Trend';

-- Спящая: гарантированно не активна.
