...
**Последнее обновление:** 2026-09-05 — **добавление/копирование логики: список разворачивается, новая логика показывается развёрнутой.** `web/src/app/logic-editor/logic-editor.component.ts`: `@Output() saved` теперь `EventEmitter<LogicRow|null>` — в `submit` отдаётся строка из ответа `createLogic`/`updateLogic`. `web/src/app/logics/logics.component.ts` `onEditorSaved(created)`: если строки с таким id нет в `this.logics` (это **новосозданная** логика) — добавляется сразу (сортировка по id), `showAllLogics = true`, `sortColumn = null`, в `expandedLogics` добавляется id, `ensureParamsDraft(created.id, true)` + `loadParamsForLogic(created.id)`, `setTimeout(scrollLogicIntoView)`. `copyLogic` теперь тоже ставит `showAllLogics = true` и сбрасывает `sortColumn` (копия `is_enabled=false` и без теста в свёрнутом списке не видна). Редактирование существующей логики — без изменений (перезагрузка списка). `ng build` prod OK (только бюджетные warning'и). Установщики **v1.0.213** (не коммичено).
