...
**Последнее обновление:** 2026-09-21 — **установщик Windows: страница 2 «Обновление MultiLogicTradePg» — вместо кнопок Да/Нет/Отмена одна кнопка «Продолжить» + чекбокс «Удалить все таблицы и их содержимое и создать всё заново» (по умолчанию ВЫКЛ = как «Нет», галочка = как «Да»).** Запрос Sergey: «на 2-й странице установщика, где Да/Нет/Отмена и где надо жать Нет, чтобы не стёрло прошлое, убери Да и Нет, замени одной кнопкой „Продолжить", добавь чекбокс „удалить все таблицы и создать заново"; чекбокс по умолчанию выключен = как Нет; если включить — то же, что делала бы кнопка Да». Решение: `installer/windows/MultiLogicTradePg.iss` — убран MsgBox(MB_YESNOCANCEL) из `InitializeSetup`; добавлена 2-я страница `GUpgradePage` (после Welcome, `CreateCustomPage(wpWelcome,…)`): поясняющий текст (версия/сборка, что данные сохраняются), чекбокс `GWipeDataCheck` (Checked=False, подпись «Удалить все таблицы и их содержимое и создать всё заново») и кнопка «Продолжить» (в `CurPageChanged` скрыт Back, `NextButton.Caption='Продолжить'`, восстановление на других страницах через `SetupMessage(msgButtonNext)`). Логика: `ShouldSkipPage` — страница только если программа установлена (`GInstalled`, определяется в InitializeSetup из реестра UninstallString); `NextButtonClick` — чекбокс отмечен → `GDbMode:='wipe'` + запуск старого деинсталлятора и проверка ResultCode, не отмечен → `GDbMode:='upgrade'`; свежая установка → `'create'`. `GetDbMode`/`PreparePostInstall`/`run_postinstall.cmd` не менялись (wipe/upgrade/create как раньше). Проверено: ISCC compile OK (успешная сборка Setup.exe). Web-код не менялся. Установщики **v1.0.258**.
