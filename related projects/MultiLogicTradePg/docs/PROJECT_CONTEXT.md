...
**Последнее обновление:** 2026-09-01 — контекст инстркций сохранён локально (#858) + выкладывание репозитория. Ранее по задаче ROC Snapback (logic 8937): корень «нет сделок в янв–апр» — в live и в корневом `02_...sql` отсутствовал движок `calc_ind_roc_array` (есть в `sql/calc_ind_extra.sql`, но не встраивался в 02) и в live была старая версия `calc_indicator_series_array` без ветки ROC; развёрнуты функции `calc_ind_roc_array`/`calc_ind_roc` и ветка ROC в оба места; добавлен chunked-загрузчик M15 (`load_prices_moex_resample_chunked` в `sql/load_prices_moex_resample.sql` + fallback в `sql/price_load_http_policy.sql`) и `scripts/backfill-m15.js`. Состав функций calc_ind_* в корневом 02 vs `sql/calc_ind_extra.sql` сверен (ROC отсутствовал — добавлено). Прошлая сводка: 2026-08-29 — после теста отчёт открывается всегда: автооткрытие в модал «Отчёты тестов» (#857) + пер-бумажные блоки (#848/#853, #856); budgets web подняты; installers — текущая версия в `VERSION.txt`.
