...
**Последнее обновление:** 2026-07-17 — fix установленного launcher v2: минимальный ASCII `.bat` + PowerShell startup logic
...
82. **Fix launcher v2:** большой `MultiLogic_Trade_Progress_Start.bat` заменён на минимальную ASCII-only обёртку; вся логика PATH/npm/cache/API/Angular перенесена в `scripts/launch_multilogictrade.ps1`, чтобы `cmd.exe` не парсил `--no-fund`, `--port`, URL и русские строки.
...
- [x] Fix launcher v2: `.bat` только ASCII-wrapper, реальный старт через PowerShell без `cmd.exe`-парсинга npm/Angular аргументов (2026-07-17).
...
| Дата | Суть |
|------|------|
| 2026-07-17 | Fix installer launcher v2: minimal ASCII bat wrapper + PowerShell launch_multilogictrade.ps1; rebuilt Setup.exe |
...
92. Повтор того же лога (`'--no-fund' is not recognized`, `Unknown argument: port`) — заменить большой `.bat` на минимальную ASCII-обёртку и перенести запуск в PowerShell.
