...
{
[Bot("AlgoStart3RzTest")]
public class AlgoStart3RzTest : BotPanel
{
...
private StrategyParameterString _trend;
private StrategyParameterDecimal _koeffPriceAdd;
...
public AlgoStart3RzTest(string name, StartProgram startProgram) : base(name, startProgram)
{
// non trade periods
...
//new
_trend = CreateParameter("Trend", "Buy on top (trend)", new[] { "Buy on top (trend)", "Sell on top (countertrend)" });
_koeffPriceAdd = CreateParameter("KoefPriceAdd", 10, 5.0m, 50, 4);
// GetVolume settings
...
DeleteEvent += AlgoStart3RzTest_DeleteEvent;
}
...
private void AlgoStart3RzTest_DeleteEvent()
{
...
if ((positions.Count == 0) /*&& (_trend.ValueString == "Buy on top (trend)")*/) // Open position logic
{
...
decimal downI = (decimal)candles[^2].Close - (_koeffPriceAdd.ValueDecimal / ((decimal)candles[^2].Close + 0.00001m)); //new
decimal upI = (decimal)candles[^2].Close + (_koeffPriceAdd.ValueDecimal / ((decimal)candles[^2].Close + 0.00001m)); //new
if (pcUp == 0)
{
return;
}
decimal candleClose = candles[candles.Count - 1].Close;
if (((candleClose > upI/*pcUp*/) && (_trend.ValueString == "Buy on top (trend)")) || ((candleClose < downI/*pcDown*/) && (_trend.ValueString == "Sell on top (countertrend)")))
{
...
if (((smaValue < smaPrev)  && (_trend.ValueString == "Buy on top (trend)"))
    || ((smaValue > smaPrev)  && (_trend.ValueString == "Sell on top (countertrend)")) )
{
return;
}
}
...
if (((lastClose <= downI/*pcDown*/) && (_trend.ValueString == "Sell on top (countertrend)")) || ((lastClose > upI/*pcUp*/) && (_trend.ValueString == "Buy on top (trend)")))
{
